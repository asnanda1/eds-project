...
/**
 * Transformer: eds-capstone-sudhansu section breaks + section metadata.
 * Applies to templates with 2+ sections (blog: 8 sections).
 *
 * Section selectors come directly from page-templates.json (DOM-verified
 * '.<block>-container' wrappers). Styles present: secondary (s1, s3, s5),
 * inverse (s8); null (s2, s4, s6, s7) -> no metadata.
 *
 * Uses both hooks: insert <hr> breaks in beforeTransform (while every section
 * element still exists, before block parsers replace them), anchor Section
 * Metadata blocks in afterTransform via a temporary marker attribute.
 */
